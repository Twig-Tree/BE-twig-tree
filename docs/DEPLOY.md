...
인스턴스가 1GB(VM.Standard.E2.1.Micro)라 컨테이너 총합을 800m 로 묶습니다. 아래 실측값은 추정이 아니라 실제로 띄워 측정한 것이고, 실서버 열은 워밍업(swagger · api-docs · 인증 필요 API 각 5회)을 마친 뒤의 값입니다.
...
컨테이너 한도 총합 800m 를 다 쓰면 1190m 가 되어 954Mi 를 넘깁니다. 한도는 어디까지나 상한이지 목표치가 아니므로, 올리기 전에 `free -h` 의 available 을 먼저 보세요.
