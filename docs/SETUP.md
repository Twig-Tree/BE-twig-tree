...
- **CORS**: 프론트 개발 서버 `http://localhost:3000`과 로컬 Swagger용 API Origin `http://localhost:8080`을 허용합니다(credentials 허용). 다른 프론트 포트가 필요하면 백엔드팀에 요청하세요.
- **인증 방식**: Access Token은 응답 JSON으로 받아 메모리에 보관하고, Refresh Token은 HttpOnly 쿠키로 관리합니다.
  ...
  - CSRF 토큰 발급 단계는 없습니다. 로그인·재발급·로그아웃 POST는 허용된 `Origin`에서만 처리하며 브라우저가 이 헤더를 자동으로 보냅니다.
